...
#define DETECT_CM    80    // 小於此距離視為偵測到物體（公分）
#define BUZZ_CM      25    // 小於此距離自動蜂鳴（公分）
#define STEP_DEG     2     // 每步轉幾度
#define STEP_MS      20    // 每步間隔（ms）
...
void handleSerial() {
    while (Serial.available()) {
        ...
        if (c == '\n') {
            cmdBuf[cmdLen] = '\0';
            if (strcmp(cmdBuf, "CMD:BUZZ") == 0)
                buzzUntil = millis() + 500;
            cmdLen = 0;
        } else if (cmdLen < 15) {
            cmdBuf[cmdLen++] = c;
        }
...
void loop() {
    ...
    digitalWrite(BUZZER, now < buzzUntil ? HIGH : LOW);
    ...
    if (now - lastStepAt >= STEP_MS) {
        ...
        if (detect) {
            ...
            tracking   = true;
            prevDetect = true;
            if (dist < BUZZ_CM) buzzUntil = now + 100;
        } else {
